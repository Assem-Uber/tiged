...
var getHomeOrTmp = () => homedir() || tmpdir();
  ...
  return new Promise((fulfil, reject) => {
    ...
    https.get(options, (response) => {
      ...
      if (code >= 400) {
        ...
        response.pipe(createWriteStream(dest)).on("finish", () => fulfil()).on("error", reject);
      }
    }).on("error", reject);
  });
}
